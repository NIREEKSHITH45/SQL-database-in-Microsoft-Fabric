...
$openAIModel1 = az cognitiveservices account deployment create -g $rgName -n $azure_open_ai --deployment-name "gpt-4" --model-name "gpt-4" --model-version "0613" --model-format OpenAI --sku-capacity 120 --sku-name "Standard" 
$openAIModel3 = az cognitiveservices account deployment create -g $rgName -n $azure_open_ai --deployment-name "text-embedding-ada-002" --model-name "text-embedding-ada-002" --model-version "2" --model-format OpenAI --sku-capacity 120 --sku-name "Standard"
...
# Variables
#$vm = "FabconVM" # replace VM name
#$SqlServer = "localhost\\FabconVM"  # Change to your SQL Server instance
#$SqlScriptPath = "C:\\Users\\$adminUsername\\Desktop\\CreateAndLoadData.sql"  # Path to your SQL script

# Run SQL script 
Invoke-Sqlcmd -ServerInstance $SqlServer -InputFile $SqlScriptPath -EncryptConnection:$false

#----------------------------------install .NET---------------------------  

## Install.net
#& $azCopyCommand copy "https://stignite24.blob.core.windows.net/dotnet/dotnet-sdk-9.0.102-win-x64.exe" "./" --recursive

# Replace with the path to your installer
#cd "C:\"
# Replace the file name if necessary
#Start-Process -FilePath ".\dotnet-sdk-9.0.102-win-x64.exe" -ArgumentList "/quiet /norestart" -Wait

# replace the path with working dir
#cd "working dir"
...
#cd ..
#cd Fabcon
#cp "artifacts/binaries/Program.cs" "C:/"
#cp "artifacts/binaries/contoso.png" "C:/GraphQLWebApp/wwwroot"
